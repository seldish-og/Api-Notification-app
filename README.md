...
```http://0.0.0.0:8000/api/get_all_clients``` - получить всех клиентов
```http://0.0.0.0:8000/api/get_all_mailing``` - получить все рассылки
```http://0.0.0.0:8000/api/statistics/get_statistics``` - получить статистику
...
•
id клиента - любые операции, которые связаны с конкретным клиентом (добавление/редактирование/отправка сообщения/…)</li>
